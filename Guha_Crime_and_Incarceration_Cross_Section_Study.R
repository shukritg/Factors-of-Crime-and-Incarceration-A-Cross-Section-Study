...
# health, conservative Advantage and Violent Crime
...
check_reg = lm(Health ~ log(Incarc_rate) + log(Poverty) + log(Unemp) + D_cont + Conserv + log(Violent.Crime.Rate),
               data = Incarc_data)
summary(check_reg)
# No, Incarceration is not significant in explaining Health. Moving on...

##

# Regress crime rates on everything
# Violent Crime
...
summary(reg_3)
# LFP, Gun Control, Corruption and Religion
...
# Drug Control, Gun Control and Religion
# Entering into Incarceration Rate Regressions
# Found that backward stepwise selection only selects Violent Crime, not Property or Total Crime

###

# So wait, Health is the most significant variable in explaining both violent crime and incarceration,
# And Violent crime is significant in explaining incarceration.
# Maybe health is endogenously affecting Violent Crime, and then through Violent Crime affecting Incarceration.
...
# Fit new model
summary(lm(log(Incarc_rate) ~ log(LFP) +Conserv +log(Violent.Crime.Rate), data = Incarc_data))
...
summary(ivreg(log(Incarc_rate) ~ log(LFP) +Conserv +log(Violent.Crime.Rate) | log(LFP) +Conserv +Health, model = T,
              data = Incarc_data), diagnostics = T)
...
# The Wu-Hausman test signifies that violent Crime is endogenous, and IV estimator is consistent over OLS
# Notice, that the significance of Violent Crime has reduced from the OLS model.

# Also, the estimate has increased in value. Thus, the effect of Health (through Violent Crime) on the Incarceration
# Rate is higher than that of Violent Crime by itself.  

## Note: We do not include Conserv as an instrument of Violent Crime despite its significance in explaining Violent
# Crime (Equaltion (2)), because doing so leads to Overidentification. How? 
...
# Poverty, Unemployment and Conservative advantage (Already in)
reg_5 = lm(Health ~ log(Poverty) + log(Unemp) + Conserv, data = Incarc_data)

# Assuming Health is Endogenous, we fit the ivreg instrumenting Health with Poverty and Unemployment
library(AER)
...
# They are good instruments; no endogeneity or overidentification so OLS is better than IV, health remains significant!
...
# the significance of Violent Crime and Conserv are both reduced by the inclusion of Health as an isntrument, but the
# estimates for Violent crime increases by more than twofold (from ~0.29 in OLS to ~0.77 in IV).


# We fitted a linear model specified by fourth_reg to obtain the same results as the subset selection. However, 
# applying the Breusch-Pagan test to our fitted model, we found hetereoscedatic errors. Correcting for them and testing 
# the coefficients again for significance revealed that conserv and Violent Crime dropped out, leaving only Health behind.
